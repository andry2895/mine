...
if (!empty($query)) {
    ...
    foreach ($rows as $i => $r) {
        ...
        foreach ($total['fields'] as $pos => $field) {
			...
			{
                 if ($id_module==21 && $id_plugin==29) $result[] = str_replace('|attr|', implode(' ', $attributes), '<div |attr|><img src="'.substr($value,0,-4).'_thumb100.jpg" type="image" class="img-thumbnail img-responsive" data-parsley-errors-container="#immagine20-errors"></div>');
				 if ($id_module==21 && $id_plugin<>29) $result[] = str_replace('|attr|', implode(' ', $attributes), '<div |attr|><img src="files/articoli/'.substr($value,0,-4).'_thumb100.jpg" type="image" class="img-thumbnail img-responsive" data-parsley-errors-container="#immagine20-errors"></div>');
				 if ($id_module==47) $result[] = str_replace('|attr|', implode(' ', $attributes), '<div |attr|><img src="'.substr($value,0,-4).'_thumb100.jpg" type="image" class="img-thumbnail img-responsive" data-parsley-errors-container="#immagine20-errors"></div>');
			} else 
			{
