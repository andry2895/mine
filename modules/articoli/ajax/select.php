...
switch ($resource) {
    ...
    case 'varianti_articoli':
	    if (isset($superselect['idarticolo'])) {
        $query = 'SELECT id, descrizione FROM mg_articoli_varianti |where|';
    ...
    case 'produttori':
        ...
        $where[] = '`idanagrafica` in (Select `idanagrafica` from `an_tipianagrafiche_anagrafiche` where `idtipoanagrafica` = 7)';
    ...
    case 'gruppoatt':
        $query = 'SELECT id, valore AS descrizione FROM mg_gruppo_attributi |where| ORDER BY valore';
    ...
    case 'attributo':
        if (isset($superselect['id_gruppo_attributi'])) {
            $query = 'SELECT id, valore AS descrizione FROM mg_attributi |where| ORDER BY valore';

            foreach ($elements as $element) {
                $filter[] = 'id='.prepare($element);
            }

            $where[] = '`id_gruppo_attributi`='.prepare($superselect['id_gruppo_attributi']);
